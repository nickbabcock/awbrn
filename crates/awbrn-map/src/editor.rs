...
impl MapEditor {
    ...
    /// The strongest fold the board already reads the same under.
    ///
    /// Several modes can hold at once — a board mirrored on both axes is also
    /// turned half a turn — so the modes are tried in order of how much they
    /// say, and the first that holds is the answer. A board that nothing has
    /// been drawn on yet reads the same under every fold and is evidence for
    /// none of them, so it is left free.
    pub fn detect_symmetry(&self) -> Symmetry {
        if self.board_is_blank() {
            return Symmetry::None;
        }

        // Detection reads strictly, where the readout reads leniently, and the
        // difference is the difference between the two questions. Detection is
        // working out which fold a map it has never seen was drawn under, and
        // the buildings are the best evidence there is: two headquarters at
        // opposite corners say more about the fold than a field of plain
        // ground ever will. Ignoring them here would open a board under a fold
        // it was never drawn under.
    ...
    /// Whether every tile, buildings included, is the image the fold asks for.
    ///
    /// This is the whole board read literally. Only [`Self::detect_symmetry`]
    /// wants it: a map maker is told about [`Self::is_symmetric`] instead,
    /// which leaves deliberate buildings alone.
    fn folds_exactly(&self, symmetry: Symmetry) -> bool {
        ...
        for (position, terrain) in self.map.iter() {
            for (step, isometry) in symmetry.isometries().iter().enumerate() {
                ...
                if found != expected_image(terrain, *isometry, &self.roster, symmetry, step) {
                    return false;
                }
            }
        }
    ...
    ///
    /// The fold is a promise about the land: the shape of the ground, the
    /// water, and the ways across both have to match tile for tile. This
    /// returns the tiles that break that promise, in board order, so the
    /// screen can say how many there are and where the first one is. A board
    /// that keeps the promise gives back nothing.
    ///
    /// Buildings and units are not read, and that is deliberate rather than
    /// unfinished. A competitive map answers the first-turn advantage with
    /// exactly the things this check would otherwise call broken — a base set
    /// where the other army takes it on turn one, a city one side reaches
    /// first, a unit given to whoever moves second. Those are the map maker's
    /// decisions. A board that flagged them would be arguing with the design
    /// instead of checking the ground, which is the one thing a fold can
    /// promise and a person cannot keep by hand.
    ///
    /// Painting still shares a building out to every army the fold seats, so
    /// the fold does the work; what it will not do is refuse a board that was
    /// then adjusted on purpose. The one exception is a pair — two buildings
    /// of the same kind facing each other across the fold — which must be in
    /// the armies the fold assigns, because a map mirrored by hand almost
    /// always leaves one headquarters in the colour it was copied from.
    ...
    fn folds_leniently(
        ...
        for (step, isometry) in symmetry.isometries().iter().enumerate() {
            ...
            match (terrain, found) {
                // A building standing on the fold itself is its own image, so
                // no army can be on the far side of it and the fold has
                // nothing to say about who holds it. A central airport belongs
                // to whoever the map maker gave it to.
                (AwbwTerrain::Property(_), _) if image == position => continue,
                // Two buildings of the same kind facing each other across the
                // fold are a pair, and a pair in the wrong colours is the
                // mistake this check exists for.
                (AwbwTerrain::Property(here), AwbwTerrain::Property(there))
                    if here.kind() == there.kind() =>
                {
                    return false;
                }
                // Anything else involving a building is a decision rather than
                // a defect. A base put where the second army takes it on turn
                // one, a city one side reaches first: these are how a map
                // answers the first-turn advantage, and the fold has no
                // business calling them broken.
    ...
    /// Whether nothing has been drawn on the board yet: one terrain everywhere,
    /// and no units.
    fn board_is_blank(&self) -> bool {
        if self.map.deployments().iter().next().is_some() {
            return false;
        }
        ...
        tiles.all(|terrain| terrain == first)
    }
...
/// The terrain an image of `terrain` should hold.
fn expected_image(
    terrain: AwbwTerrain,
    isometry: Isometry,
    roster: &ArmyRoster,
    ...
    let turned = isometry.turn_terrain(terrain);
    match turned {
        AwbwTerrain::Property(property) => {
            let Faction::Player(faction) = property.faction() else {
                return turned;
            };
            AwbwTerrain::Property(
                property.with_owner(Faction::Player(roster.rotate(faction, symmetry, step))),
            )
        }
        other => other,
    }
...
mod tests {
    ...
    #[test]
    fn units_never_decide_whether_the_ground_folds() {
        let mut drawing = editor(9, 5);
        drawing.paint(
            Pos::new(2, 2),
            Brush::Unit {
    ...
    #[test]
    fn a_headquarters_in_the_wrong_colour_is_not_a_fold() {
        let mut drawing = editor(9, 5);
        assert!(drawing.set_symmetry(Symmetry::MirrorLeftRight));
        drawing.paint(
            Pos::new(0, 0),
        ...
        // The far headquarters is repainted as a second Orange Star one, which
