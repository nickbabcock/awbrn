...
use awbrn_map::{AwbrnMapDocument, AwbrnMapMetadata, AwbwMap, Dimensions, ValidatedMapDocument};
...
use bevy::asset::io::{AssetSourceBuilder, wasm::HttpWasmAssetReader};
...
pub enum GameEvent {
    ...
    TileHoverChanged(TileHoverChanged),
...
impl BevyApp {
    ...
    pub fn new(
        ...
        if let Some(callback) = event_callback {
            ...
            wasm_sink!(TileHoverChanged, TileHoverChanged);
    ...
    pub fn editor_document(&mut self, name: String, author: String) -> Result<JsValue, JsError> {
        ...
        let document = session.document(AwbrnMapMetadata {
            name,
            author,
            player_count: 0,
        });
        // A seat is read off the board the way the map reads it back: an army
        // that owns no property but starts with a unit still holds a seat, and
        // a count that missed it would cut that army out of the seat list.
        let player_count = document
            .terrain
            .iter()
            .filter_map(|terrain| terrain.owner())
            .filter_map(|faction| match faction {
                awbrn_types::Faction::Player(player) => Some(player),
                awbrn_types::Faction::Neutral => None,
            })
            .chain(
                document
                    .units
                    .iter()
                    .map(|unit| PlayerFaction::from(unit.faction)),
            )
            .collect::<std::collections::BTreeSet<_>>()
            .len();

        let document = AwbrnMapDocument {
