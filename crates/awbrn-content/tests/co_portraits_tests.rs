...
fn co_portrait_lookup_uses_stable_keys() {
    let adder = co_portrait_by_awbw_id(11).expect("Adder should exist");
    assert_eq!(adder.key(), "adder");
    assert_eq!(adder.display_name(), "Adder");

    let von_bolt = co_portrait_by_awbw_id(30).expect("Von Bolt should exist");
    assert_eq!(von_bolt.key(), "von-bolt");
    assert_eq!(von_bolt.display_name(), "Von Bolt");

    let no_co = co_portrait_by_awbw_id(31).expect("No CO should exist");
