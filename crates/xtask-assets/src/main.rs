...
struct UnitAtlasManifest {
    ...
    frames_per_faction: u16,
    factions: Vec<UnitAtlasFactionEntry>,
    units: Vec<UnitAtlasUnitEntry>,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnitAtlasSheet {
    columns: u32,
    rows: u32,
    cell_width: u32,
    cell_height: u32,
...
fn run_units() -> Result<()> {
    ...
    let units_manifest_path = repo_root.join("assets/data/unit_atlas_manifest.json");
    let generated_dir = repo_root.join("crates/awbrn-content/src/generated");
    ...
    let unitsheet = build_spritesheet(
        &unit_frame_paths,
        &unitsheet_path,
        UNITSHEET_COLUMNS,
    ...
    write_unit_atlas_manifest(&unit_definitions, unitsheet, &units_manifest_path)?;
...
fn render_unit_animation_data(units: &[UnitDefinition], spritesheet: SpritesheetBuild) -> String {
    ...
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_COLUMNS: u32 = {};\n",
        spritesheet.columns
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_ROWS: u32 = {};\n",
        spritesheet.rows
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITE_WIDTH: u32 = {};\n",
        spritesheet.cell_width
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITE_HEIGHT: u32 = {};\n\n",
        spritesheet.cell_height
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_PADDING_X: u32 = {};\n",
        spritesheet.padding_x
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_PADDING_Y: u32 = {};\n",
        spritesheet.padding_y
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_OFFSET_X: u32 = {};\n",
        spritesheet.offset_x
    ));
    output.push_str(&format!(
        "pub const UNIT_SPRITESHEET_OFFSET_Y: u32 = {};\n\n",
        spritesheet.offset_y
    ));
    ...
    let total_frames = unit_offsets
        ...
        .checked_add(
            units
                .last()
                .map(|unit| unit_definition_total_frames(unit) as u16)
                .unwrap_or(0),
        )
        .unwrap_or_else(|| panic!("Unit animation frame count overflow"));
    output.push_str("impl UnitAnimationData {\n");
    output.push_str(&format!(
        "    pub const TOTAL_FRAMES: usize = {total_frames};\n"
    ));
    output.push_str("}\n");
    output
}
...
fn write_unit_atlas_manifest(
    ...
    let units_manifest = unit_offsets
        ...
        .collect::<Vec<_>>();
    let frames_per_faction = unit_offsets
        .last()
        .map(|(_, offset)| *offset)
        .unwrap_or(0)
        .checked_add(
    ...
    let manifest = UnitAtlasManifest {
        sheet: UnitAtlasSheet {
            columns: spritesheet.columns,
            rows: spritesheet.rows,
            cell_width: spritesheet.cell_width,
            cell_height: spritesheet.cell_height,
            padding_x: spritesheet.padding_x,
            padding_y: spritesheet.padding_y,
            offset_x: spritesheet.offset_x,
            offset_y: spritesheet.offset_y,
        },
        frames_per_faction,
        factions: UNIT_FACTIONS
            .iter()
            .enumerate()
    ...
    let content =
        serde_json::to_string_pretty(&manifest).context("Serializing unit atlas manifest")?;
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).context("Creating unit atlas manifest directory")?;
    }
    fs::write(output_path, content).context("Writing unit atlas manifest")?;
...
fn build_spritesheet(
    ...
    let gutter = bleed * 2;
    let sheet_width = cols * (max_width + gutter);
    let sheet_height = rows * (max_height + gutter);
    ...
    for (index, (_path, image)) in images.into_iter().enumerate() {
        ...
        let base_x = col * (max_width + gutter) + bleed;
        let base_y = row * (max_height + gutter) + bleed;
        ...
        if bleed > 0 {
            extrude_rect(&mut sheet, base_x, base_y, max_width, max_height, bleed);
        }
    }
...
fn extrude_rect(sheet: &mut RgbaImage, x: u32, y: u32, width: u32, height: u32, bleed: u32) {
    ...
    for offset in 1..=bleed {
        for row in top..=bottom {
            let left_pixel = *sheet.get_pixel(left, row);
            let right_pixel = *sheet.get_pixel(right, row);
            sheet.put_pixel(left - offset, row, left_pixel);
            sheet.put_pixel(right + offset, row, right_pixel);
        }

        for col in left..=right {
            let top_pixel = *sheet.get_pixel(col, top);
            let bottom_pixel = *sheet.get_pixel(col, bottom);
            sheet.put_pixel(col, top - offset, top_pixel);
            sheet.put_pixel(col, bottom + offset, bottom_pixel);
        }

        let top_left = *sheet.get_pixel(left, top);
        let top_right = *sheet.get_pixel(right, top);
        let bottom_left = *sheet.get_pixel(left, bottom);
        let bottom_right = *sheet.get_pixel(right, bottom);
        for dx in 1..=bleed {
            for dy in 1..=bleed {
                sheet.put_pixel(left - dx, top - dy, top_left);
                sheet.put_pixel(right + dx, top - dy, top_right);
                sheet.put_pixel(left - dx, bottom + dy, bottom_left);
                sheet.put_pixel(right + dx, bottom + dy, bottom_right);
            }
        }
    }
}
...
fn render_terrain_animation_data(tiles: &[TileMetadata]) -> String {
    ...
    for tile in tiles {
        ...
        // Convert timings to u16 array literal (clamp in u32 domain, then cast)
        let timings: Vec<String> = tile
            .frame_timings
            .iter()
            .map(|t| format!("{}", (*t).min(65535) as u16))
            .collect();
