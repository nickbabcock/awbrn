...
struct Animation {
    start_index: usize,
    frame_durations: Vec<u32>, // Duration in milliseconds for each frame
    current_frame: usize,
    frame_timer: Timer,
}
    ...
    for (mut animation, mut sprite) in query.iter_mut() {
        ...
        if animation.frame_timer.just_finished() {
            ...
            animation.current_frame =
                (animation.current_frame + 1) % animation.frame_durations.len();
            ...
            if let Some(next_duration) = animation.frame_durations.get(animation.current_frame) {
                animation.frame_timer = Timer::new(
                    Duration::from_millis(*next_duration as u64),
                    TimerMode::Once,
                );
            }
        }
...
fn spawn_animated_unit(
    ...
    for unit in replay_units {
        ...
        let first_frame_duration = animation_frames.frame_duration(0).unwrap_or(750);
        let animation = Animation {
            start_index: animation_frames.start_index() as usize,
            frame_durations: animation_frames
                .frame_durations()
                .iter()
                .map(|&x| x as u32)
                .collect(),
            ...
            frame_timer: Timer::new(
                Duration::from_millis(first_frame_duration as u64),
                TimerMode::Once,
            ),
        };
