use awbrn_core::{GraphicalTerrain, PlayerFaction, UnitDomain};
...
mod tests {
    ...
    fn apply_unit_vision_creates_manhattan_diamond() {
        ...
        // Center and adjacent tiles should be visible
        assert!(!map.is_fogged(Position::new(3, 3)));
        assert!(!map.is_fogged(Position::new(3, 1))); // 2 up
        assert!(!map.is_fogged(Position::new(3, 5))); // 2 down
        assert!(!map.is_fogged(Position::new(1, 3))); // 2 left
        assert!(!map.is_fogged(Position::new(5, 3))); // 2 right
        assert!(!map.is_fogged(Position::new(4, 4))); // diagonal distance 2

        // Distance 3 should be fogged
        assert!(map.is_fogged(Position::new(3, 0))); // 3 up
        assert!(map.is_fogged(Position::new(0, 3))); // 3 left
        assert!(map.is_fogged(Position::new(5, 4))); // diagonal distance 3
    }
    ...
    fn apply_unit_vision_respects_limit_distance() {
        ...
        // Wood at (4, 3) has limit=1 (only visible if adjacent)
    ...
    fn apply_unit_vision_clamps_to_map_bounds() {
        ...
        // Should reveal what's in bounds without panicking
        assert!(!map.is_fogged(Position::new(0, 0)));
        assert!(!map.is_fogged(Position::new(1, 1)));
