...
pub struct Engagement {
    ...
    /// in, so the health spread and the luck spread can be told apart. Empty
    /// when there is no spread to break out: a reply that lands first is
...
fn score(
    ...
    let counter_steps = transition::forecast_counter_steps(
        &state,
        &PlayerId::from(ATTACKING_PLAYER),
        attacker_index,
        origin,
