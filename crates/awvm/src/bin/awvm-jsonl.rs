...
use awvm::{
    semantic::{self, AwbwVisibility, PlayerId, RulesetRef, State},
    transition::{self, Command, ExecuteError},
};
...
fn main() {
    ...
    for line in stdin.lock().lines() {
        let response = match line {
            ...
            Ok(_) => continue,
            Err(e) => error("", "IO_ERROR", e.to_string()),
        };
...
fn handle(line: &str) -> Value {
    ...
    if envelope.protocol_version != "0.1.0" {
        return error(
            &envelope.request_id,
...
fn execute(line: &str, request_id: &str) -> Value {
    ...
    match transition::execute(&request.state, command, &request.random) {
        Ok(result) => {
            json!({"protocol_version":"0.1.0","request_id":request.request_id,"status":"accepted","state":result.state,"events":result.events,"random_consumed":result.random_consumed})
        }
        Err(ExecuteError::Violation(violation)) => {
            json!({"protocol_version":"0.1.0","request_id":request.request_id,"status":"rejected","violation":violation,"random_consumed":0})
        }
...
fn error(request_id: &str, code: &str, message: impl Into<String>) -> Value {
    json!({"protocol_version":"0.1.0","request_id":request_id,"status":"error","code":code,"message":message.into()})
}
