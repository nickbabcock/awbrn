...
mod tests {
    ...
    #[test]
    fn simple_fixture_executes_to_expected_snapshot() {
        let case: Value = serde_json::from_str(include_str!(
            "../../../spec/fixtures/movement/infantry-plain-move.json"
        ))
        .unwrap();
        ...
        assert_eq!(
            ...
            case["steps"][0]["expect"]["state"]
        );
        assert_eq!(
            serde_json::to_value(result.events).unwrap(),
            case["steps"][0]["expect"]["events"]
        );
    }

    #[test]
    fn movement_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/movement/infantry-plain-move.json"),
            include_str!("../../../spec/fixtures/movement/move-wait-capture-reset.json"),
            include_str!("../../../spec/fixtures/movement/repeated-position-precedence.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
                let command: Command = serde_json::from_value(step["command"].clone()).unwrap();
                let expected = &step["expect"];
                match execute(&state, command, &[]) {
                    Ok(result) => {
                        assert_eq!(expected["status"], "accepted", "{}", case["id"]);
                        assert_eq!(
                            serde_json::to_value(&result.state).unwrap(),
                            expected["state"],
                            "{}",
                        ...
                        assert_eq!(result.random_consumed, 0, "{}", case["id"]);
                        state = result.state;
                    }
                    Err(ExecuteError::Violation(actual)) => {
                        assert_eq!(expected["status"], "rejected", "{}", case["id"]);
                        assert_eq!(actual, expected["violation"], "{}", case["id"]);
                    }
                    Err(error) => panic!("{}: unexpected error {error:?}", case["id"]),
                }
            }
        }
    }
    ...
    #[test]
    fn capture_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/capture/capture-city-partial.json"),
            include_str!("../../../spec/fixtures/capture/capture-city-complete.json"),
            include_str!("../../../spec/fixtures/capture/capture-hq-victory.json"),
            include_str!("../../../spec/fixtures/capture/capture-noncapturer-rejected.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            let command: Command =
                serde_json::from_value(case["steps"][0]["command"].clone()).unwrap();
            let expected = &case["steps"][0]["expect"];
            match execute(&state, command, &[]) {
                Ok(result) => {
                    assert_eq!(expected["status"], "accepted", "{}", case["id"]);
                    assert_eq!(
    ...
    #[test]
    fn production_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/production/produce-infantry-on-base.json"),
            include_str!("../../../spec/fixtures/production/produce-insufficient-funds.json"),
            include_str!("../../../spec/fixtures/production/produce-occupied-facility.json"),
            include_str!("../../../spec/fixtures/production/produce-wrong-domain.json"),
            include_str!("../../../spec/fixtures/production/unit-limit-counts-cargo.json"),
            include_str!("../../../spec/fixtures/production/unit-limit-owner-scope.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-owned-lab.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-no-lab.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-teammate-lab.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-lost-lab.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-ban-wins.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-hachi-site.json"),
            include_str!("../../../spec/fixtures/production/lab-unit-unlisted-kind.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            let command: Command =
    ...
    #[test]
    fn join_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/join/join-after-capture-reset.json"),
            include_str!("../../../spec/fixtures/join/join-full-hp-rejected.json"),
            include_str!("../../../spec/fixtures/join/join-full-mover-into-hurt.json"),
            include_str!("../../../spec/fixtures/join/join-infantry-merge.json"),
            include_str!("../../../spec/fixtures/join/join-mech-caps-fuel-ammo.json"),
            include_str!("../../../spec/fixtures/join/join-no-overflow.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            let command: Command =
    ...
    #[test]
    fn supply_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/supply/apc-does-not-supply-teammate.json"),
            include_str!("../../../spec/fixtures/supply/apc-supplies-tank.json"),
            include_str!("../../../spec/fixtures/supply/apc-supplies-two-units.json"),
            include_str!("../../../spec/fixtures/supply/supply-no-change-emits-no-resource.json"),
            include_str!("../../../spec/fixtures/supply/supply-non-apc-rejected.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            let command: Command =
    ...
    #[test]
    fn repair_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/repair/black-boat-repairs-black-boat.json"),
            include_str!("../../../spec/fixtures/repair/repair-full-hp-resupply-only.json"),
            include_str!("../../../spec/fixtures/repair/repair-non-black-boat-rejected.json"),
            include_str!("../../../spec/fixtures/repair/repair-unaffordable-resupply-only.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            let command: Command =
    ...
    #[test]
    fn transport_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/transport/load-infantry-into-apc.json"),
            include_str!("../../../spec/fixtures/transport/load-hidden-blocker.json"),
            include_str!("../../../spec/fixtures/transport/load-validation.json"),
            include_str!("../../../spec/fixtures/transport/unload-after-transport-spent.json"),
            include_str!("../../../spec/fixtures/transport/unload-compacts-lander-cargo.json"),
            include_str!("../../../spec/fixtures/transport/unload-infantry-from-apc.json"),
            include_str!("../../../spec/fixtures/transport/unload-nonadjacent-rejected.json"),
            include_str!("../../../spec/fixtures/transport/unload-validation-fog.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn launch_explode_and_delete_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/launch/move-launch-effects.json"),
            include_str!("../../../spec/fixtures/launch/move-launch-hidden-blocker.json"),
            include_str!("../../../spec/fixtures/launch/move-launch-validation.json"),
            include_str!("../../../spec/fixtures/explode/move-explode-effects.json"),
            include_str!("../../../spec/fixtures/explode/move-explode-hidden-blocker.json"),
            include_str!("../../../spec/fixtures/explode/move-explode-validation.json"),
            include_str!("../../../spec/fixtures/delete/delete-unit-capture-reset.json"),
            include_str!("../../../spec/fixtures/delete/delete-unit-validation.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn concealment_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/concealment/hide-already-hidden-rejected.json"),
            include_str!("../../../spec/fixtures/concealment/hide-non-concealable-rejected.json"),
            include_str!("../../../spec/fixtures/concealment/reveal-already-exposed-rejected.json"),
            include_str!("../../../spec/fixtures/concealment/stealth-hides-in-place.json"),
            include_str!("../../../spec/fixtures/concealment/sub-dives.json"),
            include_str!("../../../spec/fixtures/concealment/sub-surfaces.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn power_activation_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/commander/adder-power-activation.json"),
            include_str!("../../../spec/fixtures/commander/andy-cop.json"),
            include_str!("../../../spec/fixtures/commander/eagle-scop-activation.json"),
            include_str!("../../../spec/fixtures/commander/jess-cop.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn turn_boundary_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/turn/end-turn-income-ready.json"),
            include_str!("../../../spec/fixtures/turn/round-boundary-day-advance.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn tag_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/tag/combat-dual-charge.json"),
            include_str!("../../../spec/fixtures/tag/combat-power-freezes-both-slots.json"),
            include_str!("../../../spec/fixtures/tag/swap-and-power-expiry.json"),
            include_str!("../../../spec/fixtures/tag/tag-disabled-rejected.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
                let command: Command = serde_json::from_value(step["command"].clone()).unwrap();
                let random: Vec<Value> = serde_json::from_value(step["random"].clone()).unwrap();
                let expected = &step["expect"];
                match execute(&state, command, &random) {
                    Ok(result) => {
                        assert_eq!(expected["status"], "accepted", "{}", case["id"]);
                        assert_eq!(
                            serde_json::to_value(&result.state).unwrap(),
                        ...
                        assert_eq!(
                            u64::try_from(result.random_consumed).unwrap(),
                            expected["random_consumed"].as_u64().unwrap(),
                            "{}",
                            case["id"]
                        );
                        state = result.state;
    ...
    #[test]
    fn turn_hook_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/turn-hooks/apc-auto-supply.json"),
            include_str!("../../../spec/fixtures/turn-hooks/fuel-upkeep-and-crash.json"),
            include_str!("../../../spec/fixtures/turn-hooks/property-repair.json"),
            include_str!("../../../spec/fixtures/turn-hooks/random-weather-outcomes.json"),
            include_str!("../../../spec/fixtures/turn-hooks/random-weather-override-expiry.json"),
            include_str!("../../../spec/fixtures/turn-hooks/weather-expiry.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
                let command: Command = serde_json::from_value(step["command"].clone()).unwrap();
                let random = step["random"].as_array().unwrap();
                let expected = &step["expect"];
                match execute(&state, command, random) {
                    Ok(result) => {
                        assert_eq!(expected["status"], "accepted", "{}", case["id"]);
    ...
    #[test]
    fn elimination_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/elimination/hq-capture-cascade-transfers.json"),
            include_str!("../../../spec/fixtures/elimination/resign-cascade-continues.json"),
            include_str!("../../../spec/fixtures/elimination/resign-ends-match.json"),
            include_str!("../../../spec/fixtures/elimination/resign-non-active-rejected.json"),
            include_str!("../../../spec/fixtures/elimination/rout-crash-reselects-successor.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
    ...
    #[test]
    fn combat_movement_fixtures_match_expected_results() {
        for source in [
            include_str!("../../../spec/fixtures/combat/movement-direct-fog-capture.json"),
            include_str!("../../../spec/fixtures/combat/movement-hidden-blocker.json"),
            include_str!("../../../spec/fixtures/combat/movement-tile-target.json"),
            include_str!("../../../spec/fixtures/combat/movement-validation.json"),
        ] {
            let case: Value = serde_json::from_str(source).unwrap();
            let mut state: State = serde_json::from_value(case["initial_state"].clone()).unwrap();
            for step in case["steps"].as_array().unwrap() {
