...
impl EditorSession {
    ...
    /// The map as it now stands, with its player count.
    pub fn document(&self, name: String, author: String) -> awbrn_map::AwbrnMapDocument {
        let player_count = u32::try_from(self.editor.map().factions().len())
            .expect("an AWBW map has fewer than u32 player factions");
        self.editor.document(AwbrnMapMetadata {
            name,
            author,
            player_count,
        })
...
mod tests {
    ...
    #[test]
    fn a_saved_draft_counts_factions_from_properties_and_units() {
        let mut draft = session(5, 5);
        assert_eq!(
            draft
                .document("Draft".into(), "Map maker".into())
                .metadata
                .player_count,
            0
        );
        ...
        draft.editor.paint(
            ...
            Brush::Unit {
                ...
                faction: FactionCode::new(PlayerFaction::BlueMoon),
                hp: VisualHp::new(10),
            },
        );
