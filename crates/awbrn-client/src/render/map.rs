...
impl BackdropTexturesResource {
    fn texture_for(&self, weather: awbrn_types::Weather) -> Handle<Image> {
        match weather {
            awbrn_types::Weather::Clear => self.clear.clone(),
            awbrn_types::Weather::Snow => self.snow.clone(),
            awbrn_types::Weather::Rain => self.rain.clone(),
        }
    }
}
...
pub(crate) fn initialize_backdrop_textures(
    ...
    let (clear_image, snow_image, rain_image) = {
        ...
        (
            extract_plain_backdrop_image(atlas_image, awbrn_types::Weather::Clear),
            extract_plain_backdrop_image(atlas_image, awbrn_types::Weather::Snow),
            extract_plain_backdrop_image(atlas_image, awbrn_types::Weather::Rain),
        )
    };
...
fn insert_terrain_visual(
    ...
    weather: awbrn_types::Weather,
    terrain: GraphicalTerrain,
) {
    let sprite_index = awbrn_content::spritesheet_index(weather, terrain);
    ...
    if sprite_index.animation_frames() > 1 {
        let frame_durations = awbrn_content::get_terrain_animation_frames(terrain);
...
mod tests {
    ...
    #[test]
    fn weather_change_refreshes_all_terrain_visuals() {
        let mut app = terrain_render_test_app();
        ...
        assert_eq!(
            snow_index,
            awbrn_content::spritesheet_index(Weather::Snow, GraphicalTerrain::Plain).index()
                as usize
        );
    }
