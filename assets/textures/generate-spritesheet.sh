...
montage -tile 64x -mode concatenate -gravity southeast -geometry '16x32>' -background transparent -define webp:lossless=true $(cat \
...
montage -tile 64x -mode concatenate -gravity southeast -geometry '23x24>' -background transparent -define webp:lossless=true \
    $(find "$TEXTURES_DIR/Units" -type f | sort) units.webp
