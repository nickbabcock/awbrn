...
export async function saveEditedMap(request: MapSaveRequest, actor: Actor): Promise<MapSaveResult> {
  ...
  if (held.currentRevision !== request.expectedRevision || held.name !== request.expectedName) {
    throw new Error("This map changed after you opened it. Reload the editor before saving.");
  }
  ...
  if (held.contentHash === imported.contentHash) {
    ...
    const updated = await db
      ...
      .where(
        and(
          eq(maps.id, request.mapId),
          eq(maps.currentRevision, request.expectedRevision),
          eq(maps.name, request.expectedName),
        ),
      )
      .run();
  ...
  const [, updateResult] = await db.batch([
    db
      ...
      .select(
        db
          .select({
            ...
            contentHash: sql<string>`${imported.contentHash}`.as("contentHash"),
            width: sql<number>`${document.width}`.as("width"),
            height: sql<number>`${document.height}`.as("height"),
            playerCount: sql<number>`${document.metadata.player_count}`.as("playerCount"),
            propertySignature: sql<string>`${imported.propertySignature}`.as("propertySignature"),
            unitSignature: sql<string>`${imported.unitSignature}`.as("unitSignature"),
            rank: sql<null>`NULL`.as("rank"),
            createdAt: sql<Date>`${sql.param(now, mapRevisions.createdAt)}`.as("createdAt"),
            lastSeenAt: sql<Date>`${sql.param(now, mapRevisions.lastSeenAt)}`.as("lastSeenAt"),
          })
          .from(maps)
          .where(
            and(
      ...
      .onConflictDoNothing({ target: [mapRevisions.mapId, mapRevisions.revision] }),
    db
      .update(maps)
        ...
        and(
          ...
          exists(
            db
              ...
              .where(
                and(
                  eq(mapRevisions.mapId, request.mapId),
                  eq(mapRevisions.revision, revision),
                  eq(mapRevisions.contentHash, imported.contentHash),
                ),
              ),
          ),
        ),
