...
export function MapEditorPage({
  ...
  const source = useMemo<MapEditorSource | null>(() => {
    if (sourceMapId === null) {
      return { document: null, ...MAP_EDITOR_DEFAULT_SIZE };
    }
    if (!sourceRevision.data) return null;
    return { document: sourceRevision.data, ...MAP_EDITOR_DEFAULT_SIZE };
  }, [sourceMapId, sourceRevision.data]);
  ...
  useEffect(() => {
    if (!entry) return;
    setName(mapId === undefined ? `${entry.name} (fork)` : entry.name);
  }, [entry, mapId]);

  const grant = entry ? mapEditGrant(entry, actor) : null;
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const document = await editor.readDocument(trimmed, entry?.author ?? "");
      return saveMapFn({
        data: {
          ...(intent === "revise" && mapId !== undefined ? { mapId } : {}),
          name: trimmed,
          document,
        },
      });
  ...
  const canSave = isSignedIn && name.trim().length > 0 && state !== null && !save.isPending;
        ...
        <HStack align="end" gap={4} justify="between" wrap="wrap">
          <VStack gap={0.5}>
            <Heading level={1} xstyle={styles.title}>
              {sourceMapId === null ? "New map" : (entry?.name ?? "Map")}
            </Heading>
            <Text color="secondary" type="label">
              {intent === "revise"
                ? `Editing revision ${entry?.revision ?? 1}`
                : intent === "fork"
                  ? `Forked from ${entry?.name ?? "a map"} by ${entry?.author ?? "unknown"}`
                  : "A board of your own"}
            </Text>
          </VStack>
