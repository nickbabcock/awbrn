 ...
 *
 * A request with no `mapId` writes a new map, which is what both a blank board
 * and a fork are. A request that names one writes another revision of it, and
 * the server decides whether the player may.
 */
export const mapSaveRequestSchema = z.object({
